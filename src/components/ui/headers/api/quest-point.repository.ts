...
export async function fetchQuestPoint({options}: FetchPointProps) {
    ...
    const fromDateISO = options?.from ? options.from.toISOString() :new Date(nowMilliseconds - oneWeekMs).toISOString()
    const toDateISO = options?.to ? options.to.toISOString() : currentDate.toISOString()
